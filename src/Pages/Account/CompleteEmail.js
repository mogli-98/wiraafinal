...
const CompleteEmail = ({ dataloaclhost }) => {
  ...
  const emailverify = () => {
    AuthModal.Verifyemailuser({ token })
      .then((res) => {
        if (res.data.status === true) {
          ...
          localStorage.setItem(
            "userProfileId",
            dataloaclhost.data.user.UsersProfileID
          );
          ...
          localStorage.setItem(
            "AffiliateId",
            dataloaclhost.data.user.AffiliateId
          );
          localStorage.setItem("UserType", dataloaclhost.data.user.UserType);
          localStorage.setItem(
            "AffiliateCode",
            dataloaclhost.data.user.AffiliateCode
          );
        ...
        <Grid container spacing={3}>
          ...
          <Grid sm={6} xs={12} style={{ padding: "10px" }} className="signup  ">
            <center>
              ...
              <Link to="/">
                {" "}
                <img src={wirralogo} alt="" className="m-5 " />
              </Link>{" "}
            </center>
            <Card
              className="shadow-5 login-card "
              size="lg"
              style={{
                padding: "30px",
                display: "flex",
                ...
                border: "none",
                boShadow:
                  "0 14px 26px -12px rgb(54 191 244 / 42%), 0 4px 23px 0px rgb(0 0 0 / 12%), 0 8px 10px -5px rgb(54 165 244 / 20%);",
              }}
            >
