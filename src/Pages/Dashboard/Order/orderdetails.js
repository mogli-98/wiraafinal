...
function Orderdetails() {
    ...
    useEffect(() => {
        ...
        OrderModal.Interest({ orderId }).then((response) => {
            if (response.status === 404) {
                console.log("respone.data.status")
            }
            else { setInterestedPeople(response.data) }
        }).catch((error) => {
                                ...
                                <Col sm={4}>
                                    ...
                                    <Card className='order-detail-profile   d-none d-sm-block' style={{ backgroundColor: "#efefef", borderRadius: "35px", borderStyle: 'none' }}>
                                        ...
                                        <center style={{ position: 'relative' }}>
                                            ...
                                            <Link to={`/Profiledetails/${intprofileData?.[0]?.UserID}`}><RiShareBoxLine size={30} style={{ position: 'absolute', top: 0, right: 0, margin: '10px' }}/></Link>
