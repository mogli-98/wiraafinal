...
function ProfileCard(props) {
  ...
  const fetchdata = (UsersProfileID) => {
    ...
    ProfileModal.Followunfollow({ userProfileId }, { followProfileId }).then(
      (response) => {
  ...
  return (
    ...
    >
      <center>
        <img
          src={`http://demo.wiraa.com/${data.profilePic}`}
          alt=""
      ...
      <Container>
        <Row className="mb-3">
          <Col className="square border-end end-border">
            <div
              style={{ display: isHovered1 ? " none" : "block" }}
              // onMouseEnter={() => setIsHovered1(true)}
              // onMouseLeave={() => setIsHovered1(false)}
            >
